...
SAVE(v2.3): 変更があると右下に [変更を破棄](白, 左) [保存](primary, 右) の2つを出す。
  保存: validateQuestion に不備があるときは **保存させない**。押した時点で touched を全部立て、
        赤いバーに「保存できません：<不備をスラッシュ区切り>」を出し、画面も移動しない。
        不備が無ければ従来どおり、そのグループの設問一覧へ移動する。
  変更を破棄: ConfirmDialog を経由(R6)。編集を始めた時点のスナップショットに戻す。
        スナップショットが**空**(問題文も選択肢も無い＝作った直後)なら、その問題ごと削除する。
        空のまま残すと、一覧に「（無題の問題）」の行が増えてしまうため。
