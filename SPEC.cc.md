...
ADS (v3.3, 非交渉)
  Google AdSense。**VITE_AD_CLIENT と広告ユニットIDがそろうまで、枠ごと描かない**
  （審査待ちに空の箱や「広告」の字だけが残ると、画面が壊れて見える）。
  置く場所は3つだけ: 結果画面の下 / グループ一覧の下 / 演習を始める手前の1枚。
  **演習中の画面には出さない。** 選択肢のそばの広告は規約違反にあたり、
    考えている最中の邪魔は学習そのものを止める。
  演習前の1枚(AdInterstitial): **重ねない。** ポップアップの中の広告は AdSense が認めていない。
    本文と入れ替える1枚の画面として描く。
    **いつでもすぐ閉じられること。** 右上［スキップ ✕］(>=44px・開いた瞬間にフォーカス)、
    下［▶ 演習をはじめる］、Esc のどれでも進む。秒読みで足止めしない。
    出す間隔は ADS.START_COOLDOWN_MIN（既定20分）。押すたびに出すと、続けて解く人ほど損をする。
  枠は**幅が決まってから** push する（幅0で頼むと "No slot size for availableWidth=0" で
    その枠は二度と埋まらない）。AdBanner が ResizeObserver で待つ。
  枠には「広告」と添える。中身の見出しに見せかけない。1画面に何枚も並べない。
  ads.txt は **app.quiz-make.com と quiz-make.com の両方**に要る（public/ と site/public/）。
